...
import Exam from '../model/AllExam.js';
...
export async function register(req, res) {
	try {
		...
		return res
			.status(201)
			.json({ message: 'User registered successfully.', OTP: otp });
	} catch (error) {
	...
	try {
		if (req.body) {
			...
			if (updatedUser.nModified === 0) {
				return res
					.status(404)
					.send({ error: 'User not found or no modifications made' });
			}

			const updatedUserData = await userModel.findOne({ _id: id });
			return res
				.status(200)
				.send({ msg: 'Record updated successfully', data: updatedUserData });
		}
	} catch (error) {
...
export async function verifyOTP(req, res) {
	try {
		...
		if (!user) {
			return res
				.status(404)
				.json({ error: 'User not found or OTP is incorrect.' });
		}
...
export async function updatePassword(req, res) {
	try {
		...
		if (!user) {
			...
			return res
				.status(404)
				.send({ error: 'User not found or invalid reset token' });
		}

		// Verify that the provided reset token matches the one stored in the database
		if (user.resetToken !== resetToken) {
			console.log(
				'Provided reset token does not match the stored token for email:',
				email
			);
			return res.status(400).send({ error: 'Invalid reset token' });
		}
...
export async function createContactRequest(req, res) {
	try {
		...
		if (err.name === 'ValidationError' || err.name === 'CastError') {
			...
			res
				.status(500)
				.json({ error: 'Failed to create contact request1: ' + err.message });
		}
	}
}
...
export async function getAllContactRequests(req, res) {
	try {
		...
		const contactRequests = await ContactRequest.find().populate(
			'userId',
			'name email phone'
		...
		res
			.status(500)
			.json({ error: 'Failed to retrieve contact requests: ' + err.message });
	}
}
...
export async function getContactRequestById(req, res) {
	try {
		...
		res
			.status(500)
			.json({ error: 'Failed to retrieve contact request: ' + err.message });
	}
}
